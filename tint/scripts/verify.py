...
class VerificationData(cy.selections.NTDataSpecs.NT86v5):
    _path_data = '/data/user/wluszczak/KDE_csky/test_trial_%s.npy'%(inputseed)
...
ana = cy.get_analysis(cy.selections.repo, 'version-005-p00', VerificationData, dir=ana_dir)
...
src = cy.sources(np.radians(40.667), np.radians(-0.0069))
#src = cy.sources(np.radians(77.3582), np.radians(5.69314))

#tr = cy.get_trial_runner(ana=ana, src=src, space='generic', func=calc_sb_ratio, features={'ev_ra':'ra', 'ev_dec':'dec', 'ev_angErr':'sigma', 'ev_logE':'log10energy'}, fits={'gamma':(1.0,2.0,3.0,4.0)}, extra_keep=['dec', 'sigma', 'sindec'], energy=False, cut_n_sigma=np.inf, concat_evs=True, flux=hyp.PowerLawFlux(gamma))
tr = cy.get_trial_runner(ana=ana, src=src, extra_keep=['dec', 'sigma', 'sindec'], cut_n_sigma=5., flux=hyp.PowerLawFlux(gamma))
...
#    trial_mask = dist_mask(trial[0][0][0], src, 15.)
 #   fit = tr.get_one_fit_from_trial(trial,flat=False,_masks=[trial_mask], TRUTH=True)
    fit = tr.get_one_fit_from_trial(trial, flat=False, TRUTH=True) 
    print(fit)
    t2 = time.time()
    ...
    ns_fit = fit[1]['ns']
    results.append([ts,ns_fit,gamma_fit])

np.save('/data/user/wluszczak/KDE_csky/tint/verification_noKDE_%s.npy'%(inputseed), results)
#np.save('/data/user/wluszczak/KDE_csky/tint/verification_results_%s.npy'%(inputseed), results)
#if nsig==0.:
